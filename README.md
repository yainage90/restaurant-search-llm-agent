...
1. 네이버 개발자센터에서 제공하는 API를 사용해서 검색할 키워드 수집
   - 장소명
     - Gemini-2.5-Pro 모델에 서울/경기도 행정동 이름, 서울/경기도 지하철역, 서울/경기도 번화가 지명을 뽑아달라고 요청해서 장소명 수집
   - 음식 메뉴명
     - 배달의민족, 네이버, 요기요 등에서 사람들이 주요한 메뉴명들 수집

(장소명) + (메뉴명) 모든 조합으로 네이버 지역 검색 API 호출

네이버 지역검색 API는 아래와 같이 장소명, 주소, 좌표 정도만 제공함
...
네이버 지도로 (식당명) + ('동' 까지의 행정구역) 키워드로 설명하면 웬만하면 식당 목록이 아닌 특정 식당 상세 페이지가 바로 나오기 때문에 크롤링 하기가 수월하다.
...
이것은 아직 문서의 최종 형태는 아니다. 아래에 설명이 나와있지만, 여기에 식당소개글과 사용자 리뷰를 가지고 LLM으로 추가 피쳐를 추출해서 내용을 더 풍부하게 해주고 임베딩을 추출한다.
...
내부 데이터 검색결과와 웹 검색결과는 문서 형식이 서로 다르므로 LLM에 요청하기 전 하나의 일관된 텍스트 컨텍스트로 만드는 과정이 필요하다.
